...
void xSceneSelection::showGizimo()
{
	if(m_pCurGizmo == NULL)
		return ;
    ISceneGraph* pScene = m_pEvolEnv->getCurrentScene();
    if( pScene != NULL)
    {
        m_pCurGizmo->attachToScene( pScene );
    }
...
void xSceneSelection::setRotGizmo()
{
	...
	{
		m_pRotGizimo = dynamic_cast<xSceneHelperDrawableNode*>(XEvol_CreateSceneNode(L"xSceneHelperDrawableNode" , pScene , NULL) );
