...
{
	int n = CMaxEnv::singleton().m_pInterace->GetSelNodeCount();
	bool * IsChildNodes = new bool[n];
    vector<INode*> Nodes;
	for(int i = 0 ; i < n ; i ++)
	{
		IsChildNodes[i] = false;
		char* pName = CMaxEnv::singleton().m_pInterace->GetSelNode(i)->GetName();
		Nodes.push_back(CMaxEnv::singleton().m_pInterace->GetSelNode(i));
	}
	//------------------------------------------------------------------
	for(int i = 0 ; i < n ; i ++)
	{
...
{
	INode* pNode =  CMaxEnv::singleton().m_pInterace->GetRootNode();
	CMaxNodeTreeItem TreeNode(NULL);
	size_t nSize =  m_MaxNodeTreeRoot.size();
	m_MaxNodeTreeRoot.push_back(TreeNode);
	m_MaxNodeTreeRoot[nSize].m_pMaxNode = pNode;
