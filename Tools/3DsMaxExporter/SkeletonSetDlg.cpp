...
void CSkeletonSetDlg::OnBnClickedSaveSkeleton()
{
	...
	{
		skel_exporter.build_modifiers(bones);
...
void CSkeletonSetDlg::OnBnClickedAllBones()
{
	...
	sMaxNodes_t bones;
	BuildBonesFromNodes(bones,nodes);
...
void CSkeletonSetDlg::OnBnClickedAddBones()
{
	...
	{  
		...
		{
			if(*i == CMaxEnv::singleton().m_pInterace->GetRootNode())
			{
				bones.erase(i);
				break;
			}
		...
		{
			if(CMaxEnv::singleton().m_pInterace->GetRootNode() != bones[i] )
			   CMaxEnv::singleton().SetXEvol3DBone(bones[i],TRUE);
		}
