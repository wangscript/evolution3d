...
public:
		typedef     std::vector<unsigned int> vMaxTimes;
public:
	void  export_mesh_data(xMeshData& meshData , std::wstring& mesh_dir , xcomdoc& doc , sSkeletonID_t&  sid  , bool SkinMesh , bool bTextMode );
	bool  export_mesh_action(std::wstring prefix, xcomdoc& doc , vector<sBoundBox_t>& FrameBoundBoxs , wstring mesh_name,CMaxMesh& mesh,vMaxTimes& exportTimes);
};
