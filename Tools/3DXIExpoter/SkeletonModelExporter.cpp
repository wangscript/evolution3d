...
bool CSkeletonModelExporter::__export_header(std::wstring prefix)
{
...
bool CSkeletonModelExporter::__export_skin(std::wstring prefix)
{
	sSkinGroups_t skin_groups =  m_SkinGroupBuilder.GetSkinGroups();
	for(size_t ig = 0 ; ig < skin_groups.size() ; ig++)
	{
		 ...
		 BuildSkinMeshsFromNodes(maxMeshs,max_nodes,&m_Skeleton);
		 CSkinExporter skinExporter(m_doc);
		 skinExporter.set_skeleton(&m_Skeleton);

		 std::wstring skin_dir = skin_group.m_GroupName;
