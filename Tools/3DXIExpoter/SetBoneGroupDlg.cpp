...
void CSetBoneGroupDlg::BuildContex(INode* pNode , bool bRecursive)
{
	...
	if( !pNode->UserPropExists(BONEGROUP_PRO) && G_MaxEnv().IsBone(pNode) )
	{
...
{
	Confirm();
	BuildContex(G_MaxEnv().m_pInterace->GetRootNode() );
}



int CSetBoneGroupDlg::CreateGroup(std::wstring GroupName)
{
	...
	for(size_t i = 0 ; i < m_Groups.size() ; i ++,it ++)
	{
		if(m_Groups[i].m_TreeItem == hItem)
		{
			...
			for(size_t j = 0 ; j < group.m_BoneItems.size() ; ++j)
			{
				...
				G_MaxEnv().SetXEvol3DBone(bItem.m_Bone,FALSE);
			}
			m_Groups.erase(it);
...
void CSetBoneGroupDlg::DeleteGroup(std::wstring name)
{
	 sBoneGroups_t::iterator it = m_Groups.begin();
     for(size_t i = 0 ; i < m_Groups.size() ; i ++,it ++)
	 {
...
void CSetBoneGroupDlg::InsertBone(std::wstring GroupName, INode* pNode)
{
    DeleteBone(pNode);
...
{
	wofstream out;
	G_MaxEnv().BeginLog(L"xrm_bone_group.dump.txt");
