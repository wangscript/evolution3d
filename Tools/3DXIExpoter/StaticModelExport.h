...
protected:
    ...
    bool    GetSaveFileName(wchar_t* file_name);
    
    
    bool    export_to_file(wchar_t* file_name);
    bool    export_to_package(wchar_t* file_name);

    bool    export_header(std::wstring prefix,vector<wstring>& mesh_names);
    bool    export_mesh(std::wstring prefix,wstring mesh_name,CMaxMesh& mesh , bool bTextMode);

	wstring get_avialabale_mesh_name(std::wstring prefix, std::wstring name);
