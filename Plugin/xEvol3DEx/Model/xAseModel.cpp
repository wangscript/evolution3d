...
IMPL_BASE_OBJECT_CLASSID(xAseModel , xBaseModel);
IMPL_BASE_OBJECT_CLASSID(xAseMesh  , xCoreMesh );
...
bool xAseMesh::load(tMaterialInfo* pMat , t3DObject* pObject , xGeomLib::xaabb& _aabb)
{ 
...
//------------------------
xAseModel::xAseModel(xBaseTextureMgr* pTexMgr , bool bSysMemCopy)
...
//====================
bool xAseModel::load(const wchar_t* fileName , const unsigned int8* buf , size_t bufLen, unsigned long arg)
{
	m_aabb.Init(100000000.0f);
	...
	{
        CLoadASE loader;
		if( false == loader.ImportASE( &model , fileName) )
			return false;
	}
	//if(_ext == L"3ds")
	//{
	//	CLoad3DS loader;
	//	if( false == loader.Import3DS( &model , fileName) )
	//		return false;
	//}
	...
	{
          xAseMesh * pMesh = new xAseMesh(m_pTexMgr , m_bSysMemCopy);
...
bool xAseModel::_loadModelMeshMateril( xCoreMesh* mesh , xXmlNode* pMeshNode)
{
	return false;
}

bool xAseModel::load(xcomdoc& doc , const wchar_t* _dir , unsigned int arg)
{
    //xcomdoc[_dir.3ds] 目前放在xcomdoc里的模型不支持。
	return false;
}

void xAseModel::_setupMesh( xCoreMesh* pMesh, xXmlNode*pMeshNode ) 
{
	_loadModelMeshMateril(pMesh,pMeshNode);
	m_Meshs.push_back(pMesh);
}

bool xAseModel::loadSkeleton(xcomdoc& doc , const wchar_t* _dir)
{
	//没有骨骼动画的啦
	m_pSkeleton = NULL;    
	return false;
