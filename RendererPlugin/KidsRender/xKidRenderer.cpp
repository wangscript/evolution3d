...
xMaterial* xKidRenderer::createMaterial(const wchar_t* matName)
{
	...
	xKidBasicMaterial* pKidMat = new xKidBasicMaterial(this);
...
size_t xKidRenderer::nRenderEffect()
{
	return 2;
}
...
{
    xXmlDocument doc;
	std::wstring matListFile = _XEVOL_ABSPATH_(L"./KidRenderer/materials.xml");
...
const wchar_t* xKidRenderer::materialName(int idx)
{
	...
	if(idx >= (int)nMat)
		return NULL;
	static std::wstring _matName = L"";
	_matName = matNodes[idx]->value(L"name");
	doc.unload();
