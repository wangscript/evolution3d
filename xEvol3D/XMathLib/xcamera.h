...
public:
	...
	public:
		...
		xMathLib::xvec4 getDir(bool bNormalized = true);
		xMathLib::xvec4 getRight();
	};
...
public:
	...
	*****************************************************************/
	...
	xMathLib::xvec3	   trans2Screen(xMathLib::xvec3& vScr , const xMathLib::xvec3& pvPos );
	void	           getPickRay(xMathLib::xvec3& pvPickRay,int x,int y,int sw,int sh);
	void	           getPickRay(xline& pvPickLine, int x, int y, int sw=-1, int sh=-1);
	void               cloneTo(xCamera* pCamera , bool fireEvent = false );
    void               calcBillboardMatrix(const xMathLib::xmat4& worldMatrix , xMathLib::xmat4& _bbMatrix);
protected:
	void        __setdirty();
	void        __calcMatrix();
	/*
