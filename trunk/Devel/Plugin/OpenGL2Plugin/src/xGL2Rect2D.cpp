...
xGL2Rect2DEnv::xGL2Rect2DEnv(xGL2RenderApi* pApi)
{
...
float xGL2Rect2DEnv::zValue()
{
	return m_pRenderApi->get2DZValue() ; 
}


bool xGL2Rect2DEnv::create()
{
	for(int i = 0 ; i <= MAX_UV_CHANEL; i ++)
	{
		...
		wsprintf(_iisName , L"xGL2Rect%d" , i);
...
xGL2Rect2D::xGL2Rect2D(xGL2Rect2DEnv*  pRectEnv)
{
	m_bDirty   = false;
	m_pRectEnv = pRectEnv;
...
void xGL2Rect2D::setTexture(int iStage , IBaseTexture* pTexture)
{
...
void xGL2Rect2D::setTexture(int iStage , IBaseTexture* pTexture , float _srcRect[4])
{
...
void xGL2Rect2D::setRect(float _destRect[4])
{
...
void xGL2Rect2D::commit()
{
	...
	m_pVertexBuffer->update(eLock_WriteDiscard , m_Vertex , sizeof(xGL22DVertex) * 4);

	m_bDirty = false;
	return ;
}

void xGL2Rect2D::create()
{
...
void xGL2Rect2D::setUVChanel(int iUVChanel)
{
	...
	wchar_t _iisName[16] = {0};
	wsprintf(_iisName , L"xGL2Rect%d" , iUVChanel );
	m_pInputAss = m_pRectEnv->m_pRenderApi->getInputAssembler(_iisName);
}

int xGL2Rect2D::nUVChanel()
{
	return m_nUVChanel;
}

xGL2Rect2D::~xGL2Rect2D()
{
...
bool xGL2Rect2D::draw()
{
	...
	m_pRectEnv->m_pRenderApi->setVertexBuffer(m_pVertexBuffer , 0 , sizeof(xGL22DVertex) );
