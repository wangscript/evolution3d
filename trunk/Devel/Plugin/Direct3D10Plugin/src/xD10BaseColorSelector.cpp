...
bool xD10BaseColorSelector::setRenderObjectID(int id1 , int id2)
{
...
bool xD10BaseColorSelector::setRenderObjMasterID(int _id)
{
	m_SelectID[0]._id = _id;
	return true;
}

bool xD10BaseColorSelector::setRenderObjSlaveID(int _id)
{
	m_SelectID[1]._id = _id;
	return true;
}

xD10BaseColorSelector::xD10BaseColorSelector(xD3D10RenderApi* pD10Api) : IColorSelector(pD10Api)
{
	m_pD10Api = pD10Api;
	m_SelectID[0]._id = 0; 
	m_SelectID[1]._id = 0;
...
xD10BaseColorSelector::~xD10BaseColorSelector()
{
...
bool xD10BaseColorSelector::create(xXmlNode* pSelNode)
{
	...
	m_SelectRenderView = (xD10RenderView*) m_pD10Api->createRenderView( 0 ,0 , true);
	m_SelectRenderView->createRenderTarget(1, fmt , true ,false);
	m_Format = fmt;
	m_pColorSelShader = (xD10Shader*)m_pD10Api->createShader(pColorShader,eShader_PixelShader);
	m_ColorSelBlender = m_pD10Api->createBlendState(L"ColorSel");
	return true;
}

bool xD10BaseColorSelector::begin()
{
...
bool xD10BaseColorSelector::resize(int w , int h )
{
...
bool xD10BaseColorSelector::end()
{
	if(m_SelectRenderView == NULL)
		return false;
...
bool xD10BaseColorSelector::getSelectID(int x , int y , int w , int h, void* _data)
{
...
bool xD10BaseColorSelector::beginPrimitive()
{
	m_pD10Api->setPixelShader(m_pColorSelShader);
	xD10PixelShader* pColorSelShader = (xD10PixelShader*)m_pColorSelShader;
	pColorSelShader->commitParamTable();
	return true;
}

bool xD10BaseColorSelector::endPrimitive()
{
	return true;
}
