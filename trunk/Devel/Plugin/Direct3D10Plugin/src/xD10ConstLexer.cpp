...
void xD10ConstLexer::__addDXGIFormat(ePIXEL_FORMAT fmt , DXGI_FORMAT dxfmt , const wchar_t* descStr , int nCompont , int _byte )
{
    xD10GIFormatInfo fmtInfo;
...
xD10GIFormatInfo* xD10ConstLexer::GetPixelFormat( DXGI_FORMAT dxfmt )
{
...
xD10GIFormatInfo* xD10ConstLexer::GetPixelFormat( ePIXEL_FORMAT fmt )
{
...
xD10ConstLexer::xD10ConstLexer()
{
		...
		//__addDXGIFormat(PIXELFORMAT_ ,DXGI_FORMAT_BC1_UNORM	= 71,
		//__addDXGIFormat(PIXELFORMAT_ ,DXGI_FORMAT_BC1_UNORM_SRGB	= 72,
		//__addDXGIFormat(PIXELFORMAT_ ,DXGI_FORMAT_BC2_TYPELESS	= 73,
		//__addDXGIFormat(PIXELFORMAT_ ,DXGI_FORMAT_BC2_UNORM	= 74,
		//__addDXGIFormat(PIXELFORMAT_ ,DXGI_FORMAT_BC2_UNORM_SRGB	= 75,
		//__addDXGIFormat(PIXELFORMAT_ ,DXGI_FORMAT_BC3_TYPELESS	= 76,
		//__addDXGIFormat(PIXELFORMAT_ ,DXGI_FORMAT_BC3_UNORM	= 77,
		//__addDXGIFormat(PIXELFORMAT_ ,DXGI_FORMAT_BC3_UNORM_SRGB	= 78,
		//__addDXGIFormat(PIXELFORMAT_ ,DXGI_FORMAT_BC4_TYPELESS	= 79,
		//__addDXGIFormat(PIXELFORMAT_ ,DXGI_FORMAT_BC4_UNORM	= 80,
		//__addDXGIFormat(PIXELFORMAT_ ,DXGI_FORMAT_BC4_SNORM	= 81,
		//__addDXGIFormat(PIXELFORMAT_ ,DXGI_FORMAT_BC5_TYPELESS	= 82,
		//__addDXGIFormat(PIXELFORMAT_ ,DXGI_FORMAT_BC5_UNORM	= 83,
		//__addDXGIFormat(PIXELFORMAT_ ,DXGI_FORMAT_BC5_SNORM	= 84,
...
xD10ConstLexer* xD10ConstLexer::singleton()
{
	static xD10ConstLexer g_s;
	return &g_s;
}

D3D10_FILL_MODE xD10ConstLexer::GetFillMode(const wchar_t* _fill)
{
	if(_fill == NULL) _fill = L"fill";
	std::wstring fillMode = _fill ;
	if(fillMode == L"line" || fillMode == L"wireframe") return D3D10_FILL_WIREFRAME	;
	if(fillMode == L"solid"|| fillMode == L"fill" )     return D3D10_FILL_SOLID	;
	 return D3D10_FILL_SOLID	;
}

D3D10_CULL_MODE xD10ConstLexer::GetCullMode(const wchar_t* _cull)
{	
	if(_cull == NULL) _cull = L"none";
    std::wstring cullMode = _cull ;
	if(cullMode == L"none")            return  D3D10_CULL_NONE;
	if(cullMode == L"front")            return  D3D10_CULL_FRONT;
	if(cullMode == L"back")            return  D3D10_CULL_BACK;
	return  D3D10_CULL_BACK;
}

D3D10_BLEND_OP  xD10ConstLexer::GetBlendOp(const wchar_t* _blenop)
{
	if(_blenop == NULL) _blenop = L"add";
	std::wstring blendop = _blenop ;
	if(blendop == L"add")            return  D3D10_BLEND_OP_ADD;
	if(blendop == L"subtract")       return  D3D10_BLEND_OP_SUBTRACT;
	if(blendop == L"rev_subtract")   return  D3D10_BLEND_OP_REV_SUBTRACT;
	if(blendop == L"min")            return  D3D10_BLEND_OP_MIN;
	if(blendop == L"max")            return  D3D10_BLEND_OP_MAX;
	return D3D10_BLEND_OP_ADD;
}

D3D10_BLEND  xD10ConstLexer::GetBlendFactor(const wchar_t* _blend)
{
	if(_blend == NULL) _blend = L"one";
	std::wstring blend = _blend ;
	if(blend == L"zero")            return  D3D10_BLEND_ZERO;
	if(blend == L"one")             return  D3D10_BLEND_ONE;
	if(blend == L"src")             return  D3D10_BLEND_SRC_COLOR;
	if(blend == L"inv_src")         return  D3D10_BLEND_INV_SRC_COLOR;
	if(blend == L"src_alpha")       return  D3D10_BLEND_SRC_ALPHA;
	if(blend == L"inv_src_alpha")   return 	D3D10_BLEND_INV_SRC_ALPHA;
	if(blend == L"dest_alpha")      return 	D3D10_BLEND_DEST_ALPHA;
	if(blend == L"inv_dest_alpha")  return 	D3D10_BLEND_INV_DEST_ALPHA;
	if(blend == L"dest")            return 	D3D10_BLEND_DEST_COLOR;
	if(blend == L"inv_dest")        return 	D3D10_BLEND_INV_DEST_COLOR;
	if(blend == L"src_alpha_sat")   return 	D3D10_BLEND_SRC_ALPHA_SAT;
	if(blend == L"factor")          return 	D3D10_BLEND_BLEND_FACTOR;
	if(blend == L"inv_factor")      return 	D3D10_BLEND_INV_BLEND_FACTOR;
	if(blend == L"src1")            return 	D3D10_BLEND_SRC1_COLOR;
	if(blend == L"inv_src1")        return 	D3D10_BLEND_INV_SRC1_COLOR;
	if(blend == L"src1_alpha")      return 	D3D10_BLEND_SRC1_ALPHA;
	if(blend == L"src1_alpha")      return 	D3D10_BLEND_INV_SRC1_ALPHA;
	return  D3D10_BLEND_ONE;
}

int xD10ConstLexer::GetTextureSlotIdx(const wchar_t* _texName)
{
...
D3D10_FILTER xD10ConstLexer::GetFilter(const wchar_t* _min_val , const wchar_t* _mag_val , const wchar_t* _mip_val ,  bool bCamp)
{
	...
	D3D10_FILTER filter = D3D10_FILTER_MIN_MAG_MIP_POINT;
	if(min_val == L"anisotropic" || min_val == L"aniso" || mag_val == L"anisotropic" || mag_val == L"aniso")
	{
		filter = D3D10_FILTER_ANISOTROPIC;
		if(bCamp)
	...
	D3D10_FILTER a[2][2][2][2];
	a[0][0][0][0] = D3D10_FILTER_MIN_MAG_MIP_POINT;
	a[0][0][0][1] = D3D10_FILTER_MIN_MAG_POINT_MIP_LINEAR ;
	a[0][0][1][0] = D3D10_FILTER_MIN_POINT_MAG_LINEAR_MIP_POINT;
	a[0][0][1][1] = D3D10_FILTER_MIN_POINT_MAG_MIP_LINEAR;
	a[0][1][0][0] = D3D10_FILTER_MIN_LINEAR_MAG_MIP_POINT;
	a[0][1][0][1] = D3D10_FILTER_MIN_LINEAR_MAG_POINT_MIP_LINEAR;
	a[0][1][1][0] = D3D10_FILTER_MIN_MAG_LINEAR_MIP_POINT;
	a[0][1][1][1] = D3D10_FILTER_MIN_MAG_MIP_LINEAR;


	a[1][0][0][0] = D3D10_FILTER_COMPARISON_MIN_MAG_MIP_POINT;
	a[1][0][0][1] = D3D10_FILTER_COMPARISON_MIN_MAG_POINT_MIP_LINEAR;
	a[1][0][1][0] = D3D10_FILTER_COMPARISON_MIN_POINT_MAG_LINEAR_MIP_POINT;
	a[1][0][1][1] = D3D10_FILTER_COMPARISON_MIN_POINT_MAG_MIP_LINEAR;
	a[1][1][0][0] = D3D10_FILTER_COMPARISON_MIN_LINEAR_MAG_MIP_POINT;
	a[1][1][0][1] = D3D10_FILTER_COMPARISON_MIN_LINEAR_MAG_POINT_MIP_LINEAR;
	a[1][1][1][0] = D3D10_FILTER_COMPARISON_MIN_MAG_LINEAR_MIP_POINT;
	a[1][1][1][1] = D3D10_FILTER_COMPARISON_MIN_MAG_MIP_LINEAR;

	return a[iComp][iMin][iMag][iMip];
}


D3D10_TEXTURE_ADDRESS_MODE xD10ConstLexer::GetAdress(const wchar_t* _addressMode)
{
	if(_addressMode == NULL) _addressMode = L"clamp";
	std::wstring addressMode = _addressMode ;
	if(addressMode == L"wrap")       return D3D10_TEXTURE_ADDRESS_WRAP;
	if(addressMode == L"mirror")     return D3D10_TEXTURE_ADDRESS_MIRROR ;
	if(addressMode == L"clamp")      return D3D10_TEXTURE_ADDRESS_CLAMP ;
	if(addressMode == L"border")     return D3D10_TEXTURE_ADDRESS_BORDER ;
	if(addressMode == L"mirroronce") return D3D10_TEXTURE_ADDRESS_MIRROR_ONCE ;
	return D3D10_TEXTURE_ADDRESS_CLAMP;
}

D3D10_COMPARISON_FUNC xD10ConstLexer::GetCompareFunc(const wchar_t* _func)
{
	if(_func == NULL) _func = L"never";
	std::wstring func = _func ;
	if(func == L"never")   return D3D10_COMPARISON_NEVER;
	if(func == L"less")    return D3D10_COMPARISON_LESS;
	if(func == L"equal")   return D3D10_COMPARISON_EQUAL;
	if(func == L"lequal")  return D3D10_COMPARISON_LESS_EQUAL;
	if(func == L"greator") return D3D10_COMPARISON_GREATER;
	if(func == L"nequale") return D3D10_COMPARISON_NOT_EQUAL;
	if(func == L"gequale") return D3D10_COMPARISON_GREATER_EQUAL;
	if(func == L"always")  return D3D10_COMPARISON_ALWAYS;
	return D3D10_COMPARISON_NEVER;
}

D3D10_STENCIL_OP xD10ConstLexer::GetStencilOp(const wchar_t* _op)
{
	if(_op == NULL) _op = L"never";
	std::wstring func = _op ;
	if(func == L"keep")       return D3D10_STENCIL_OP_KEEP;
	if(func == L"zero")       return D3D10_STENCIL_OP_ZERO;
	if(func == L"replace")    return D3D10_STENCIL_OP_REPLACE;
	if(func == L"incr_sat")   return D3D10_STENCIL_OP_INCR_SAT;
	if(func == L"decr_sat")   return D3D10_STENCIL_OP_DECR_SAT;
	if(func == L"invert")     return D3D10_STENCIL_OP_INVERT;
	if(func == L"increase")   return D3D10_STENCIL_OP_INCR;
	if(func == L"decrease")   return D3D10_STENCIL_OP_DECR;

	if(func == L"incr")       return D3D10_STENCIL_OP_INCR;
	if(func == L"decr")       return D3D10_STENCIL_OP_DECR;
