...
bool xD3D10RenderApi::getWindowSize(int& w , int & h)
{
    RECT _Rect;
...
bool xD3D10RenderApi::getRenderViewSize(int& w , int & h)
{
...
bool xD3D10RenderApi::__createD10Device()
{
      ...
      HMODULE hDriverModule[]=
      {
            ...
            LoadLibraryW( _ABSPATH(L"D3D10WARP_beta.DLL") ),
            LoadLibraryA( "D3D10WARP.DLL" ),
            LoadLibraryW( _ABSPATH(L"D3D10WARP.DLL" ) ),
            NULL,
      };
      ...
      for(int i = 0 ; i < vAdapters.size() ; i ++)
      {
            ...
            for( UINT driverTypeIndex = 0; driverTypeIndex < numDriverTypes; driverTypeIndex++ )
            {
                  ...
                  hr = D3D10CreateDeviceAndSwapChain( pAdapter, m_driverType, hModule , createDeviceFlags, D3D10_SDK_VERSION, &m_swapChainDesc, &m_pSwapChain, &m_pD3D10Device );
                  if( SUCCEEDED( hr ) )
                        break;
            }
            if(m_pD3D10Device && m_pSwapChain)
            {
                  break;
            }
      }
...
bool xD3D10RenderApi::removeInputBuffer(IInputBuffer* pConstBuffer)
{
      if(pConstBuffer)
      {
...
bool xD3D10RenderApi::removeInputBuffer(const wchar_t* _name)
{
      if(_name)
      {
...
{
      xD10LightingEnv* pLightingEnv = new xD10LightingEnv(this , _name);
      m_objManager.LightEnvMgr().add(_name , pLightingEnv);
      return pLightingEnv;
}

ILightingEnv* xD3D10RenderApi::findLightingState(const wchar_t* _name)
{
      ILightingEnv* pLightingEnv = m_objManager.LightEnvMgr().find(_name );
      return pLightingEnv;
}

bool xD3D10RenderApi::removeLightingState(const wchar_t* _name)
{
