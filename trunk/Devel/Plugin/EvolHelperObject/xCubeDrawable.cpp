...
bool xCubeDrawable::init(IRenderApi* pRenderApi , xBaseTextureMgr* pTexMgr)
{
	struct SimpleVertex
	{
		xvec4 pos;
	...
	m_pTexture = m_pRenderApi->createFileTexture(_ABSPATH(L"texture/Cover.jpg") , NULL, 0);
	//m_pRenderApi->setShaderResource(eShader_PixelShader,0,m_pTexture);
	m_pRenderApi->setTexture(Texture_Diffuse , m_pTexture);

	m_pMaskTexture = m_pRenderApi->createFileTexture(_ABSPATH(L"texture/Mask.dds") , NULL, 0);
