...
#define ADD_FORMAT(fmt , type , bpp , pixelformat) gs_PIXEL_FORMAT.insert( IMAGEFORMAT_MAP::value_type(sILImageType(fmt , type , bpp) , pixelformat ) )
		...
		ADD_FORMAT(IL_RGB  , IL_SHORT           , 48 , PIXELFORMAT_R16G16B16);
		ADD_FORMAT(IL_RGB  , IL_UNSIGNED_SHORT  , 48 , PIXELFORMAT_R16G16B16);
	
		ADD_FORMAT(IL_RGBA , IL_BYTE            , 32 , PIXELFORMAT_R8G8B8A8);
		ADD_FORMAT(IL_RGBA , IL_UNSIGNED_BYTE   , 32 , PIXELFORMAT_R8G8B8A8);
		ADD_FORMAT(IL_RGBA , IL_SHORT           , 64 , PIXELFORMAT_R16G16B16A16);
		ADD_FORMAT(IL_RGBA , IL_UNSIGNED_SHORT  , 64 , PIXELFORMAT_R16G16B16A16);


		ADD_FORMAT(IL_BGR  , IL_BYTE            , 24 , PIXELFORMAT_B8G8R8);
		ADD_FORMAT(IL_BGR  , IL_UNSIGNED_BYTE   , 24 , PIXELFORMAT_B8G8R8);
		ADD_FORMAT(IL_BGR  , IL_SHORT           , 48 , PIXELFORMAT_B16G16R16);
		ADD_FORMAT(IL_BGR  , IL_UNSIGNED_SHORT  , 48 , PIXELFORMAT_B16G16R16);

		ADD_FORMAT(IL_BGRA , IL_BYTE            , 32 , PIXELFORMAT_B8G8R8A8);
		ADD_FORMAT(IL_BGRA , IL_UNSIGNED_BYTE   , 32 , PIXELFORMAT_B8G8R8A8);
		ADD_FORMAT(IL_BGRA , IL_SHORT           , 64 , PIXELFORMAT_B16G16R16A16);
		ADD_FORMAT(IL_BGRA , IL_UNSIGNED_SHORT  , 64 , PIXELFORMAT_B16G16R16A16);

		//Float .
		ADD_FORMAT(IL_BGR  , IL_FLOAT            , 48 , PIXELFORMAT_B16G16R16F);
		ADD_FORMAT(IL_BGR  , IL_FLOAT            , 96 , PIXELFORMAT_B32G32R32F);

		ADD_FORMAT(IL_BGRA , IL_FLOAT            , 64 , PIXELFORMAT_B16G16R16A16F);
		ADD_FORMAT(IL_BGRA , IL_FLOAT            , 128, PIXELFORMAT_B32G32R32A32F);

		ADD_FORMAT(IL_RGB  , IL_FLOAT            , 48 , PIXELFORMAT_R16G16B16F);
		ADD_FORMAT(IL_RGB  , IL_FLOAT            , 96 , PIXELFORMAT_R32G32B32F);

		ADD_FORMAT(IL_RGBA , IL_FLOAT            , 64 , PIXELFORMAT_R16G16B16A16F);
		ADD_FORMAT(IL_RGBA , IL_FLOAT            , 128, PIXELFORMAT_R32G32B32A32F);
...
bool  xILImage::load(const wchar_t* wcsFileName, const int8* buf,int buf_len, IMAGE_ORIGIN img_origin )
{
	...
	XEvol_UnicodeToLocale(wcsFileName , ansiFileName , 1024);
...
{
	char ansiFileName[1024] = {0};
	XEvol_UnicodeToLocale(filename , ansiFileName , 1024);
