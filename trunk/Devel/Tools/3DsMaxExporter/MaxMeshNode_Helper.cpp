...
bool operator == (const MaxNormal& lhv , const MaxNormal& rhv)
{
	...
	//面不相同，法向量肯定不相同
	if(lhv.iFace != rhv.iFace)
		return false;
	//只要有一个法向量是指定的，就不相同
	if(lhv.bSpecified == true || rhv.bSpecified == true)
		return false;
	//只要有一个是属于平滑组的，那么他们肯定不相同
	if(lhv.smgroup !=0 || rhv.smgroup !=0)
		return false;
	return true;
}
...
wstring getTextureName(wstring Name)
{
...
int  CMaxMesh::__add_smgrp_normal(const MaxNormal& max_normal)
{
...
void CMaxMesh::__build_smgrp_normal()
{
    ...
    for(int i = 0 ; i < m_pMesh->numFaces; ++i)
    {
        ...
        for(int iv = 0 ; iv < 3 ; ++iv)
        {
            ...
            NormalFace.normal[iv] = __add_smgrp_normal(mmormal);
        }
...
这样可以避免重复顶点
***/
int      CMaxMesh::__insert_vertex_index(sVeretIndex_t& new_vi)
{
...
sFace_t  CMaxMesh::__add_face(sMaxFace_t& face)
{
    ...
    for(int i = 0 ; i < 3 ; ++i)
    {
        ...
        retFace.vert[i] = __insert_vertex_index(Vi);
    }
...
bool CMaxMesh::_build_face_info()
{
    ...
    typedef vector<sMaxFace_t> FaceList;
...
bool CMaxMesh::__extract_vertex_weight_fromlinkednode(INode* pFakeBone, CSkeletonExporter* pSkeleton)
{
    ...
    wstring BoneName = INodeName( pFakeBone );
