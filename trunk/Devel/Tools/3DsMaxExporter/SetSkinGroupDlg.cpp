...
void CBuildSkinGroup::BuildContex(bool bUseAllNode)
{
	...
	{
		sMaxNodes_t nodes;
		BuildNoesFromSelectedNode(nodes);
		BuildMeshNodesFromNodes(mesh_nodes,nodes);
		nodes.clear();
	}
	else
	{
		BuildMeshNodesFromScene(mesh_nodes);
	}
...
int CBuildSkinGroup::CreateGroup(wstring GroupName)
{
...
void CBuildSkinGroup::DeleteGroup(wstring name)
{
...
void CBuildSkinGroup::InsertSkinMesh(wstring GroupName, INode* pNode)
{
...
int CSetSkinGroupDlg::CreateGroup(wstring GroupName)
{
...
void CSetSkinGroupDlg::DeleteGroup(wstring name)
{
	sSkinGroups_t::iterator it = m_Groups.begin();
	for(size_t i = 0 ; i < m_Groups.size() ; i ++,it ++)
	{
...
void CSetSkinGroupDlg::InsertSkinMesh(wstring GroupName, INode* pNode)
{
	DeleteSkinMesh(pNode);
	int iGroup = CreateGroup(GroupName);
	sSkinGroup_t& group = m_Groups[iGroup];
...
{
	ofstream out;
	CMaxEnv::singleton().BeginLog(L"xrm_skin_group.dump.txt");
