...
bool  CMaxMesh::build_face_info(bool forTest)
{
    ...
    if(nMat <= 0)
    {
		wstring errorString = wstring(L"应用到[") + wstring(INodeName(m_pNode)) + L"]上的材质个数为0"; 
    ...
    for(int i = 0 ; i  < nMat - 1 ; ++i)
    {
        ...
        {
            wstring errorString = wstring(L"应用到[") + wstring(INodeName(m_pNode)) + L"]的多重子材质的UV Map不相同"; 
            XEVOL_LOG(eXL_DEBUG_HIGH , L"   {Error} %s\n" , errorString.c_str() );
            MessageBoxW(GetActiveWindow(),errorString.c_str(),L"错误",MB_OK);
            return false;
        }
    ...
    {
        wstring errorString = wstring(L"应用到[") + wstring(INodeName(m_pNode)) + L"]上的材质的Actived Channel Map数目大于实际用到的ChannelMap"; 
        XEVOL_LOG(eXL_DEBUG_HIGH , L"   {Error} %s\n" , errorString.c_str() );
    ...
    {
        wstring errorString = wstring(L"应用到[") + wstring(INodeName(m_pNode)) + L"]上的材质的Channel Map没被Enable"; 
        XEVOL_LOG(eXL_DEBUG_HIGH , L"   {Error} %s\n" , errorString.c_str() );
        MessageBoxW(GetActiveWindow(),errorString.c_str(),L"错误",MB_OK);
        return false;
    }
    ...
    {
        wstring errorString = wstring(L"应用到[") + wstring(INodeName(m_pNode)) + L"]上的材质的有效材质个数为0"; 
        XEVOL_LOG(eXL_DEBUG_HIGH , L"   {Error} %s\n" , errorString.c_str() );
        MessageBoxW(GetActiveWindow(),errorString.c_str(),L"错误",MB_OK);
        return false;
    }
...
bool  CMaxMesh::build_data_info(bool bSkin)
{
    ...
    for(int i = 0 ; i < nVert ; ++i)
    {
        ...
        for(int iMapCh = 0 ; iMapCh < Vi.m_nUV ; ++iMapCh)
        {
            ...
            sUVCoord_t tv;
            tv.u = max_vertex.x; tv.v = max_vertex.y ;
