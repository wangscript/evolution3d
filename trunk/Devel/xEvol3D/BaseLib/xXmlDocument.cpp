...
//====================================================
template<typename STREAMTYPE> class _UNICODESTREAM
{
	STREAMTYPE&    _stream;
public:

	_UNICODESTREAM(STREAMTYPE& os,bool bAddHeader = false):_stream(os)
	{
		if(bAddHeader)
		{
			unsigned char buf[2];
...
template<> class _UNICODESTREAM<std::wostringstream>
{
	std::wostringstream&    _stream;
public:

	_UNICODESTREAM(std::wostringstream& os,bool bAddHeader = false):_stream(os)
	{
	}
...
template <typename STREAMTYPE > void saveToStream(bool bSaveAsXML , STREAMTYPE& _stream , xXmlNode* pNode,int level)
{
	xXmlNode::XmlValues values;

	_UNICODESTREAM<STREAMTYPE> stream(_stream);
	...
	if(bSaveAsXML == false && pNode->countCData() == 0)
	{
		...
		{
			saveToStream(bSaveAsXML, _stream,nodes[i] , level + 1);
		}
...
void xXmlDocument::__save(std::ostream& _stream , xXmlNode* pNode,int level)
{
	return saveToStream(m_saveAsXML , _stream , pNode , level);
}

void xXmlDocument::__save(std::wostringstream& _stream , xXmlNode* pNode,int level)
{
	return saveToStream(m_saveAsXML , _stream , pNode , level);
}


bool xXmlDocument::save(std::ostream& _stream, bool saveHeader)
{	
	_UNICODESTREAM<std::ostream> stream(_stream,true);

	if(saveHeader && !m_saveAsXML)
	{
		stream<< L"<?xXmlDocument version=\"1.0\" encoding=\"unicode\"?>";
		stream.endl();
	}
	else if( m_saveAsXML)
	{
...
{
	_UNICODESTREAM<std::wostringstream> stream(_stream,true);
