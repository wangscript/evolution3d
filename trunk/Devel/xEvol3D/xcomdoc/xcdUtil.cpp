...
void create_all_dir(const wchar_t* bas_dir,const wchar_t* file)
{
	...
	for(int i = 0 ; i < (int) wcslen(file) ; ++i)
	{
		if(file[i] == '/' || file[i] == '\\')
		{
			...
			dir_name[i] = 0;
			wstring dir_full_name = wstring(bas_dir) + dir_name;
...
void ExtractToDirectory(IComDocBase* _doc,const wchar_t* bas_dir)
{
	...
	for(int i = 0 ; i < (int)pDoc->m_DataEntrys.size(); ++i)
	{
		...
		{
			wstring file_name = wstring(_bas_dir) + entry.m_DataEntryItem.getDataName();
