...
public:
    ...
    ~xcomdoc();
public:
    bool                open(IReadStream*     pReader   , xcd_rwmode mode = xcdm_read , int _offset = 0 , bool load_to_mem = false);
    bool                open(std::istream*    stream    , xcd_rwmode mode = xcdm_read , int _offset = 0 , bool load_to_mem = false);
    bool                open(xcomdocstream*   pStream   , xcd_rwmode mode             , int _offset = 0 , bool load_to_mem = false);
