...
bool __flipY(FIBITMAP* pFreeImage)
{
	int h     = FreeImage_GetHeight(pFreeImage);
	int w     = FreeImage_GetWidth(pFreeImage);
	int pitch = FreeImage_GetPitch(pFreeImage);
    BYTE* pTemp   = new BYTE[ pitch ];
	BYTE* pTop    = FreeImage_GetBits(pFreeImage);
	BYTE* pBottom = pTop + pitch * (h-1);
	for(int y = 0 ; y < h / 2 ; y ++)
	{
...
bool xFreeImage::data(const int8 * image_buffer,int mWidth,int mHeight , ePIXEL_FORMAT format)
{
	 ...
	 memcpy(_data , image_buffer , mHeight * _size.pitch );
	 return true;
}
...
bool xFreeImage::convert(ePIXEL_FORMAT newFmt)
{
	if(m_pFreeImage == NULL)
		return false;
	ePIXEL_FORMAT old_fmt = __GetImagePixelFmt();
	...
	//需要翻转R/B的
	if( (old_fmt == PIXELFORMAT_B8G8R8 && newFmt == PIXELFORMAT_R8G8B8A8) || 
		(old_fmt == PIXELFORMAT_R8G8B8 && newFmt == PIXELFORMAT_B8G8R8A8)    )
	{
		__flipRGB();
	...
	if( (old_fmt == PIXELFORMAT_B8G8R8 && newFmt == PIXELFORMAT_R8G8B8) || (newFmt == PIXELFORMAT_B8G8R8 && old_fmt == PIXELFORMAT_R8G8B8) )
	{
		return __flipRGB();
	}

	if( (old_fmt == PIXELFORMAT_B8G8R8A8 && newFmt == PIXELFORMAT_R8G8B8A8) || (newFmt == PIXELFORMAT_B8G8R8A8 && old_fmt == PIXELFORMAT_R8G8B8A8) )
	{
		return __flipRGBA();
	}
	...
	//不需要翻转RGB的
	//需要翻转R/B的
	if(old_fmt == PIXELFORMAT_R8G8B8 && newFmt == PIXELFORMAT_R8G8B8A8)
	{
		FIBITMAP* pBitMap = FreeImage_ConvertTo32Bits(m_pFreeImage);
		return __replace(pBitMap);
	}
...
//增加加载器
class xFreeImageLoader : public xImageLib::xImageLoader
{
public:
...
static xFreeImageLoader loader;
const bool bFreeImageLoader     = xImageLib::registeImageLoader(L"any" , &loader);
