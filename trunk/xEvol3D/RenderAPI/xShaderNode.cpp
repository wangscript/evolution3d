...
extern void AddImportLib( ds_string& _code, ds_wstring_vector &vLibImportSet, xShaderCodeNodeMgr* pNodeMgr );
bool xShaderCodeNode::generateCode(std::ds_string& code, xShaderCodeNodeMgr* pNodeMgr )
{
    if(m_ImportNode.size() != 0)
    {
        AddImportLib(code , m_ImportNode , pNodeMgr);
    }
...
bool xShaderCodeNode::generateCode(std::ds_string& code)
{
    ...
    size_t nSections =  m_vSections.size() ;
...
xShaderCodeNode::~xShaderCodeNode()
{
    size_t nSockets = m_vSockets.size() ;
...
ds_wstring_vector&   xShaderCodeNode::importLibs()
{
    return m_ImportNode;
}
