...
class _XEVOL_BASE_API_  IInputBuffer : public IRenderApiObject
{
	IMPL_BASE_OBJECT_CLASSID_INTERFACE(IInputBuffer);
public:
	IInputBuffer(IRenderApi* pOwner):IRenderApiObject(pOwner){}
	virtual ~IInputBuffer(){};
	virtual bool                    create(size_t nElement , size_t bytePerVertex , const xInputBufferDesc* pDesc , void* pInitData = NULL  ) = 0;
	...
	virtual size_t                  nElement() = 0;
	virtual size_t                  bufLen() = 0;
};


class _XEVOL_BASE_API_  IVertexStream : public IRenderApiObject
{
public:
	IMPL_BASE_OBJECT_CLASSID_INTERFACE(IVertexStream);
	IVertexStream(IRenderApi* pOwner):IRenderApiObject(pOwner){}
...
class _XEVOL_BASE_API_  IInputAssembler : public IRenderApiObject
{
	IMPL_BASE_OBJECT_CLASSID_INTERFACE(IInputAssembler);
public:
	IInputAssembler(IRenderApi* pOwner):IRenderApiObject(pOwner){}
