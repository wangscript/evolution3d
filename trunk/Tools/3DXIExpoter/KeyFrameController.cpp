...
void KeyTest() 
{
	...
	if (!G_MaxEnv().m_pInterace->GetSelNodeCount()) 
		return;
	n = G_MaxEnv().m_pInterace->GetSelNode(0); 
...
void SampleController(INode *n, Control *c)
{
	...
	int s = G_MaxEnv().m_pInterace->GetAnimRange().Start()/tpf; 
	int e = G_MaxEnv().m_pInterace->GetAnimRange().End()/tpf;
