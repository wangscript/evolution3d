...
bool CStaticModelExporter::export()
{
    ...
    file_name[0] = 0;
    if(false == GetSaveFileName(file_name) )
    {
...
bool CStaticModelExporter::export_header(std::wstring prefix,vector<wstring>& mesh_names)
{
...
bool CStaticModelExporter::export_mesh(std::wstring prefix,wstring mesh_name,CMaxMesh& mesh , bool bTextMode)
{
...
bool CStaticModelExporter::export_to_file(wchar_t* file_name)
{
    ...
    unsigned int cur_time = G_MaxEnv().GetCurrentTime();
	G_SetCurrentTime(cur_time);
    ...
    for(size_t i = 0 ; i < m_MaxMeshs.size(); ++i)
    {
         ...
         xMeshData& meshData = mesh;
		 mesh.SetScale(m_fModelScale);
         //建立这个mesh
         if(false == mesh.CreateFaces() || false == mesh.CreateMesh( cur_time , NULL) )
		 {
			 errostring = std::wstring(L"物体[")+INodeName(mesh.m_pNode)+L"]创建GameMesh数据出错";
         ...
         //简历这个Mesh的面的信息
         if(false == mesh.BuildVertexData(NULL) )
         {
             errostring = std::wstring(L"物体[")+INodeName(mesh.m_pNode)+L"]创建顶点数据出错";
             XEVOL_LOG(eXL_DEBUG_HIGH , L"   {Error} %s\n" , errostring.c_str() );
             mesh.Free();
             continue;
         }

		 if(true == mesh.BuildMorphTarget())
		 {
         ...
         //为了防止重名，得到一个可用的名字
         std::wstring mesh_name = get_avialabale_mesh_name(L"",INodeName(mesh.m_pNode));
         //把当前的包围盒添加到模型的包围盒中
         ...
         if(m_isTextMode)
         {
              ...
              XEVOL_LOG(eXL_DEBUG_HIGH , L"       nFace =%d ;  nSubMesh = %d",meshData.m_MeshDesc.m_nFace , meshData.m_MeshDesc.m_nSubMesh );
         }
         else
         {
