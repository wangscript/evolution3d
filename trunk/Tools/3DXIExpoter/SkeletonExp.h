...
public:
	...
	int        AddSkinModifier(IGameSkin* pSkinModifer);
	int        AddSkinModifier(INode* pNode);
	int        AddSkinModifiers(sMaxNodes_t nodes);
	int        AddSkinModifiers(bool selected);
	void       RemoveModifiers();

	void       AddAllSkinModifers(INode* pNode);	
	int        FindSkinModifierIndx(INode* pNode);
	IGameSkin* FindSkinModifier(INode* pNode);
public:
	CSkeletonExporter()
	{
		m_fScale   = 1.0f;
	...
	typedef std::vector<IGameSkin*>       vGameSkinModifiers;
	typedef std::vector<sMaxBoneNode_t>   sMaxBoneNodes_t;
