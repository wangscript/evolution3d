...
bool CSkinExporter::__remove_exist_mesh(std::wstring skin_group_dir,MaxMeshs& mesh_in ,MaxMeshs& mesh_out)
{
...
bool CSkinExporter::export_skin(const std::wstring prefix, bool bTexMode ,  MaxMeshs& maxMeshs_in,  const std::wstring& SkinName,sBoundBox_t& BoundBox)
{
    ...
    for(size_t i = 0 ; i < maxMeshs.size(); ++i)
    {
        ...
        {
            errostring = std::wstring(L"物体[")+ INodeName(mesh.m_pNode) + L"]创建SkinData的信息出错";
            XEVOL_LOG(eXL_DEBUG_HIGH , L"   {Error} %s\n" , errostring.c_str() );
            mesh.free();
            continue;
        }
