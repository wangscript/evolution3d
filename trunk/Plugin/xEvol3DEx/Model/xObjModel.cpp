...
IMPL_BASE_OBJECT_CLASSID(xOBJModel , xBaseModel);
...
bool xOBJMesh::load(tMaterialInfo* pMat , t3DObject* pObject , xGeomLib::xaabb& _aabb)
{ 
	m_aabb.Init(1000000000.0f);
	xCoreMesh::unload();
	wchar_t _name[256] = {0};
...
//------------------------
xOBJModel::xOBJModel(xBaseTextureMgr* pTexMgr , bool bSysMemCopy)
...
//====================
bool xOBJModel::load(const wchar_t* fileName , const unsigned int8* buf , size_t bufLen, unsigned long arg)
{
	...
	{
        CLoadOBJ loader;
		if( false == loader.ImportOBJ( &model , fileName) )
			return false;
	}
	...
	{
          xOBJMesh * pMesh = new xOBJMesh(m_pTexMgr , m_bSysMemCopy);
...
bool xOBJModel::_loadModelMeshMateril( xCoreMesh* mesh , xXmlNode* pMeshNode)
{
	return false;
}

bool xOBJModel::load(xcomdoc& doc , const wchar_t* _dir , unsigned int arg)
{
    //xcomdoc[_dir.3ds] 目前放在xcomdoc里的模型不支持。
	return false;
}

void xOBJModel::_setupMesh( xCoreMesh* pMesh, xXmlNode*pMeshNode ) 
{
	_loadModelMeshMateril(pMesh,pMeshNode);
	m_Meshs.push_back(pMesh);
}

bool xOBJModel::loadSkeleton(xcomdoc& doc , const wchar_t* _dir)
{
	//没有骨骼动画的啦
	m_pSkeleton = NULL;    
	return false;
