...
IMPL_BASE_OBJECT_CLASSID(xD3D9TextureCube  , xD3D9BaseTexture);


xD3D9TextureCube::xD3D9TextureCube(xD3D9RenderApi* pAPI , bool bReadable)
...
xD3D9TextureCube::~xD3D9TextureCube()
{
	unload();
}

void* xD3D9TextureCube::handle()
{
	return m_pCubeTexture;
}

bool xD3D9TextureCube::isLoaded()
{
	return m_pCubeTexture != NULL ;
}

bool xD3D9TextureCube::unload()
{
	m_texBuffer.unload();
	XSAFE_RELEASE(m_pCubeTexture);
	XSAFE_RELEASE(m_pTexture);
	return !isLoaded();
}


bool xD3D9TextureCube::load(const wchar_t* fileName , unsigned long  arg)
{
...
bool xD3D9TextureCube::load(const wchar_t* fileName , const unsigned int8* buf , size_t bufLen, unsigned long arg)
{
	unload();
	return false;
}
void  xD3D9TextureCube::onDeviceReset(bool bNewDevice , ID3D9DeviceLostListener::eResetAction _Action)
{
...
bool  xD3D9TextureCube::create(const  xTextureInitDesc& initDesc , xTextureInitData* pInitData, int nInitData) 
{ 
    unload();
    m_InitDesc = initDesc;
    ...
    return _createCubeTexture(initDesc , pInitData , nInitData);
}


bool  xD3D9TextureCube::_createCubeTexture(const  xTextureInitDesc& initDesc , xTextureInitData* pInitData, int nInitData)
{
    ...
    if(FAILED(m_pD3D9RenderApi->getDevice()->CreateCubeTexture( (UINT)m_TexInfo.m_TexWidth , m_TexInfo.m_MipmapLevel , usage , m_TexInfo.m_ResFmt , pool , &m_pCubeTexture,0)) )
        return false;
    m_pTexture = m_pCubeTexture;
    ...
    if(m_TexInfo.m_D3DPool != D3DPOOL_DEFAULT)
    {
        ...
        m_pCubeTexture->GetCubeMapSurface((D3DCUBEMAP_FACES)0 , 0 , &pSurf);
    ...
    for(int iSlice = 0 ; iSlice < 6 ; iSlice ++)
    {
        ...
        {
            update( _initData->m_pData , _initData->m_DataLen , _initData->m_Pitch , _initData->m_DepthPitch , iLevel , iSlice  );
            _initData ++ ;
        }
    }
	return bRet ;
...
bool  xD3D9TextureCube::update(void* data  , int dateLen , int rowPitch , int depthPicth , int mipmapLevel , int arraySlice)
{
	if(m_pTexture == NULL)
		return false;
    IDirect3DDevice9*  pDevice = m_pD3D9RenderApi->getDevice();
    IDirect3DSurface9* pDstSurf = NULL;
    m_pCubeTexture->GetCubeMapSurface( (D3DCUBEMAP_FACES)arraySlice , mipmapLevel , &pDstSurf);
...
bool xD3D9TextureCube::lock(eLockPolicy lockPolicy, xTextureLockArea& lockInfo  , int mipmapLevel , int arraySlice)
{
	if(m_pTexture == NULL)
		return false;
    ...
    IDirect3DDevice9* pDevice = m_pD3D9RenderApi->getDevice();
    m_pCubeTexture->GetCubeMapSurface((D3DCUBEMAP_FACES)arraySlice , mipmapLevel , &m_pLockedSurface);    
    m_pCubeTexture->GetLevelDesc(mipmapLevel , &SurfDesc);
...
bool xD3D9TextureCube::unlock(xTextureLockArea& lockInfo)
{
	IDirect3DDevice9* pDevice = m_pD3D9RenderApi->getDevice();
...
IRenderTarget* xD3D9TextureCube::toRenderTarget(size_t iSlice  , size_t iMipMapLevel )
{
    xD3D9DynamicRenderTarget* pRenderTarget = new xD3D9DynamicRenderTarget(m_pD3D9RenderApi , iSlice , iMipMapLevel);
...
bool xD3D9TextureCube::grabRenderTagetData(void* pData , int x , int y , int w , int h  , int arraySlice , int mipmapLevel )
{

    IDirect3DDevice9* pDevice  = m_pD3D9RenderApi->getDevice();
    IDirect3DSurface9* pRTSurface = NULL;  
    this->m_pCubeTexture->GetCubeMapSurface((D3DCUBEMAP_FACES)arraySlice , mipmapLevel , &pRTSurface);
...
bool xD3D9TextureCube::_create3DTexture(const  xTextureInitDesc& initDesc , xTextureInitData* pInitData, int nInitData)
{
