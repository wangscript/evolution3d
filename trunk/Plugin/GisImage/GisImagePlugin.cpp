...
class xGisImagePlugin : public IPluginObject
{
public:
	xGisImagePlugin()
	{
	...
	{
		wcsncpy(info->m_Description ,  L"xGisImage Plugin name=[xGisImage]" , 256);
		info->m_ID = xStringHash(L"xGisImage Plugin");
		wcsncpy(info->m_Name , L"xGisImage Plugin" , 32);
...
{
	static xGisImagePlugin gPluginObject;
