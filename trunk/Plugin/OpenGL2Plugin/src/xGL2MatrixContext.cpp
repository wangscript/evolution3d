...
xGL2MatrixContext::xGL2MatrixContext() :
...
bool xGL2MatrixContext::init(xGL2RenderApi* pRenderApi)
{
	//注册变量绑定器
	//WorldMatrix
...
bool xGL2MatrixContext::multiMatrix(const float* _mat , eMatrixMode matMode)
{
	xmat4 matTemp;
...
bool xGL2MatrixContext::setMatrix(const float* _mat   , eMatrixMode matMode)
{
	xmat4& mat = *( (xmat4*)_mat ) ;
	switch(matMode)
	{
...
bool xGL2MatrixContext::identityMatrix(eMatrixMode matMode)
{
	switch(matMode)
	{
	case MATRIXMODE_World:
...
bool xGL2MatrixContext::getMatrix(float* _mat , eMatrixMode matMode)
{
	xmat4& mat = *( (xmat4*)_mat ) ;
	switch(matMode)
	{
...
bool xGL2MatrixContext::pushMatrix(eMatrixMode matMode)
{
	switch(matMode)
	{
	case MATRIXMODE_World:
...
bool xGL2MatrixContext::popMatrix(eMatrixMode matMode)
{
	switch(matMode)
	{
	case MATRIXMODE_World:
...
IRenderCamera* xGL2MatrixContext::createCamera(const wchar_t* cameraName)
{
      return new xGL2Camera();
}

bool xGL2MatrixContext::applyCamera()
{
	if(m_pCamera == NULL )
	{
		return false;
...
bool xGL2MatrixContext::applyCamera(IRenderCamera* pCamera)
{
	if(pCamera == NULL )
	{
		return false;
	}
	m_pCamera = (xGL2Camera *)pCamera;
	//D3DXMatrixPerspectiveFovRH( (D3DXMATRIX*)m_matProject.data , m_pCamera->getFOV(), m_pCamera->getAspect() , m_pCamera->getNearPlan(), m_pCamera->getFarPlan() );
	//D3DXMatrixLookAtRH((D3DXMATRIX*)m_matView.data , (D3DXVECTOR3*)m_pCamera->m_Eye.m_EyePos.v ,   (D3DXVECTOR3*)m_pCamera->m_Eye.m_EyeTarget.v ,  (D3DXVECTOR3*)m_pCamera->m_Eye.m_Up.v );
	m_pCamera->toProjectMatrix( m_matProject );
	m_pCamera->toViewMatrix( m_matView );
...
IRenderCamera* xGL2MatrixContext::getCamera()
{
	return m_pCamera;
}
