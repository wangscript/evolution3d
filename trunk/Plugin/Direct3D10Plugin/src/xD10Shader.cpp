...
IMPL_BASE_OBJECT_CLASSID(xD10ShaderParamTable , IShaderParamTable);
...
bool xD10VertexShader::load(const wchar_t* fileName , const unsigned int8* buf , size_t bufLen, unsigned long arg)
{
    ...
    HRESULT  hr = D3DX10CompileFromMemory((LPCSTR)buf,bufLen,NULL,pMacroDef,pInclude,func_name,"vs_4_0",dwShaderFlags, dwEffectFlags, pThreadPump , &pShaderCode,&pErroMsg  , NULL);
...
bool xD10PixelShader::load(const wchar_t* fileName , const unsigned int8* buf , size_t bufLen, unsigned long arg)
{
	...
	ID3DX10ThreadPump* pThreadPump = NULL;
	DWORD dwEffectFlags = 0;
	HRESULT  hr = D3DX10CompileFromMemory((LPCSTR)buf,bufLen,NULL,pMacroDef,pInclude,func_name,"ps_4_0",dwShaderFlags,dwEffectFlags, pThreadPump , &pShaderCode,&pErroMsg  , NULL );
...
bool xD10GeomShader::load(const wchar_t* fileName , const unsigned int8* buf , size_t bufLen, unsigned long arg)
{
	...
	ID3DX10ThreadPump* pThreadPump = NULL;
	DWORD dwEffectFlags = 0;
	HRESULT  hr = D3DX10CompileFromMemory((LPCSTR)buf,bufLen,NULL,pMacroDef,pInclude,func_name,"gs_4_0",dwShaderFlags, dwEffectFlags, pThreadPump , &pShaderCode,&pErroMsg , NULL);
