...
void xD10ConstLexer::__addDXGIFormat(ePIXEL_FORMAT fmt , DXGI_FORMAT dxfmt , const wchar_t* descStr , int nCompont , int _byte , int cpRate)
{
...
D3DX10_IMAGE_FILE_FORMAT  xD10ConstLexer::GetDX10ImageFileFormat(const wchar_t* fileName)
{
	std::ds_wstring ext = xFileSystem::singleton()->getFileExtName(fileName);
	if(ext == L"bmp") return   D3DX10_IFF_BMP;
	if(ext == L"jpg") return   D3DX10_IFF_JPG;  if(ext == L"jpeg") return   D3DX10_IFF_JPG;
	if(ext == L"png") return   D3DX10_IFF_PNG;
	if(ext == L"dds") return   D3DX10_IFF_DDS;
	if(ext == L"tif") return   D3DX10_IFF_TIFF; if(ext == L"tiff") return   D3DX10_IFF_TIFF;
	if(ext == L"gif") return   D3DX10_IFF_GIF;
	return (D3DX10_IMAGE_FILE_FORMAT)-1;
}
...
D3D10_TEXTURE_ADDRESS_MODE xD10ConstLexer::GetAdress(const wchar_t* _addressMode)
{
	...
	if(addressMode == L"repeat")     return D3D10_TEXTURE_ADDRESS_WRAP;
